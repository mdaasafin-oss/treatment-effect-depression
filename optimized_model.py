...
class DiffusionDenoiser(nn.Module):
    def __init__(self, dc: int, ds: int, n_treatments: int,
                 t_emb_dim: int, step_emb_dim: int, hidden_dims, dropout: float = 0.0):
        super().__init__()
...
class FullModel(nn.Module):
    def __init__(self, cfg: Config):
        ...
        self.denoiser   = DiffusionDenoiser(cfg.dc, cfg.ds, cfg.n_treatments,
                                             ...
                                             cfg.diff_hidden, cfg.diff_dropout)
        self.cfg = cfg
...
def stability_loss(S1: torch.Tensor, S2: torch.Tensor) -> torch.Tensor:
    return F.mse_loss(S1, S2)


def propensity_loss(logits: torch.Tensor, T: torch.Tensor) -> torch.Tensor:
...
def mmd_loss(C: torch.Tensor, T: torch.Tensor,
             n_treatments: int, sigma: float = 1.0) -> torch.Tensor:
...
def ite_loss(ite_pred: torch.Tensor, tau_target: torch.Tensor) -> torch.Tensor:
    return F.mse_loss(ite_pred, tau_target)


def vicreg_reg(S: torch.Tensor, gamma: float = 1.0,
               v_weight: float = 25.0, c_weight: float = 1.0) -> torch.Tensor:
    B, d       = S.shape
...
def diffusion_loss(denoiser, C0: torch.Tensor, S: torch.Tensor, T: torch.Tensor,
                   T_diff: int, alphas_bar: torch.Tensor, device) -> torch.Tensor:
    B   = C0.shape[0]
...
def stage1_loss(batch, model, baseline_t: int, cfg: Config, device):
...
#  6. TRAINING
# ─────────────────────────────────────────────────────────────────────────────
def train_stage1(model, train_dataset, val_dataset,
                  baseline_t: int, cfg: Config, device):
    ...
    optimiser = torch.optim.Adam(s1_params, lr=cfg.lr,
    ...
    for epoch in range(1, cfg.max_epochs_s1 + 1):
        ...
        model.train()
        train_comp = {k: 0.0 for k in
                      ['out', 'stab', 'prop', 'bal', 'ite', 'reg', 'total']}

        for batch in train_loader:
            optimiser.zero_grad()
            loss, comp = stage1_loss(batch, model, baseline_t, cfg, device)
            loss.backward()
            nn.utils.clip_grad_norm_(s1_params, cfg.grad_clip)
            optimiser.step()
            for k in comp:
                train_comp[k] += comp[k]
        ...
        with torch.no_grad():
            for X, T, y in val_loader:
                ...
                S, C    = model.encoder(X)
                y_pred  = model.outcome(S, C, T)
                val_mse += ((y_pred - y) ** 2).mean().item()
        val_mse /= len(val_loader)

        history.append({
            'epoch': epoch,
            'train_total': train_comp['total'],
            'val_mse': val_mse,
...
def train_stage2(model, train_dataset, val_dataset, cfg: Config, device):
    train_loader = DataLoader(train_dataset, batch_size=cfg.batch_size,
                              shuffle=True, drop_last=True)
    val_loader   = DataLoader(val_dataset,  batch_size=cfg.batch_size,
                              shuffle=False)
    ...
    optimiser  = torch.optim.AdamW(model.denoiser.parameters(),
                                   lr=cfg.s2_lr, weight_decay=cfg.s2_weight_decay)
    alphas_bar = make_alphas_bar(cfg.T_diff, cfg.beta_start, cfg.beta_end, device)

    total_steps = len(train_loader) * cfg.s2_max_epochs
    warmup_steps = len(train_loader) * cfg.s2_warmup_epochs
    ...
    print("  Optimizer: AdamW + Cosine schedule")
    for epoch in range(1, cfg.s2_max_epochs + 1):
        ...
        for X, T, _ in train_loader:
            ...
            loss.backward()
            nn.utils.clip_grad_norm_(model.denoiser.parameters(), cfg.grad_clip)
            optimiser.step()
            scheduler.step()
        ...
        if epoch % 10 == 0:
            print(f"  Epoch {epoch:3d} | train_diff={train_loss:.4f} | "
                  f"val_diff={val_loss:.4f} | lr={scheduler.get_last_lr()[0]:.2e}")
        ...
        if val_loss < best_val_loss - 1e-5:
            ...
            patience_ctr += 1
            if patience_ctr >= cfg.s2_patience:
                print(f"  Early stop at epoch {epoch}.")
                break
...
# ─────────────────────────────────────────────────────────────────────────────
@torch.no_grad()
def evaluate_factual(model, dataset, cfg: Config, device, use_tta: bool = True) -> dict:
    ...
    for X, T, y in loader:
        ...
        if use_tta and cfg.tta_n_views > 1:
            tta_preds = [y_hat]
            for _ in range(cfg.tta_n_views - 1):
                noise = torch.randn_like(X) * cfg.noise_sigma
                mask = (torch.rand_like(X) > cfg.feat_drop_p).float()
                X_aug = (X + noise) * mask
                S_aug, C_aug = model.encoder(X_aug)
                tta_preds.append(model.outcome(S_aug, C_aug, T))
            y_hat = torch.stack(tta_preds).mean(dim=0)
...
@torch.no_grad()
def generate_counterfactual(model, X: torch.Tensor, T_cf: torch.Tensor,
                             cfg: Config, device) -> torch.Tensor:
    model.eval()
    X, T_cf    = X.to(device), T_cf.to(device)
    ...
    for t in reversed(range(1, cfg.T_diff + 1)):
        ...
        coef2 = (1.0 - alpha_t) / torch.sqrt(1.0 - ab_t)
        C     = coef1 * (C - coef2 * eps_pred)

        if t > 1:
            sigma = torch.sqrt((1 - ab_prev) / (1 - ab_t) * betas[t - 1])
            C    += sigma * torch.randn_like(C)

    return model.outcome(S, C, T_cf)


def print_results(split_name: str, metrics: dict):
    print(f"\n  {split_name} Results:")
...
# ─────────────────────────────────────────────────────────────────────────────
#  8. TREATMENT EFFECT ANALYSIS — Objective (ii)  [FIXED]
# ─────────────────────────────────────────────────────────────────────────────
@torch.no_grad()
def analyze_ite(model, dataset, therapy_map, baseline_t, cfg, device):
    ...
    all_ite  = torch.cat(all_ite, dim=0).numpy()          # (N, K)
...
def main():
    ...
    print(f"Stage 1: lr={cfg.lr}, dropout={cfg.dropout}, Adam")
    print(f"Stage 2: AdamW+cosine, lr={cfg.s2_lr}")
    print(f"TTA: {cfg.tta_n_views} views")
    ...
    print("\n Optimized Model Results (Factual Prediction)")
    for name, ds in [('Train', train_ds), ('Val', val_ds), ('Test', test_ds)]:
        metrics = evaluate_factual(model, ds, cfg, device, use_tta=True)
        print_results(name, metrics)

    print("\n Without TTA (for comparison)")
    for name, ds in [('Test', test_ds)]:
        metrics = evaluate_factual(model, ds, cfg, device, use_tta=False)
        print_results(name, metrics)
    ...
    print("\nModel saved → optimized_model.pt")
